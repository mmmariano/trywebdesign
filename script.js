...
// Função para inicializar componentes e ícones
function initApp() {
    // 1. Renderiza os componentes (se você estiver usando uma função para injetar o HTML)
    document.getElementById('navbar').innerHTML = navbarHTML;

    // 2. Cria os ícones do Lucide (Obrigatório para os ícones aparecerem)
    if (typeof lucide !== 'undefined') {
        lucide.createIcons();
    }

    // 3. Ativa o GSAP na Navbar para entrada suave
...
function setupMobileMenu() {
    ...
    toggle.addEventListener('click', () => {
        ...
        if (isActive) {
            // Animação de entrada dos links (stagger)
            gsap.to(links, {
                y: 0,
                opacity: 1,
                stagger: 0.1,
...
// Chame a função após o DOM carregar e o componente ser injetado
document.addEventListener('DOMContentLoaded', () => {
    // Se você injeta o HTML via JS, garanta que a função rode depois da injeção
